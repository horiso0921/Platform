        ...
        def do_POST(self):
            ...
            if self.path == '/dialog_end':
                ...
                try:
                    ...
                    with open(f"../log/per/{body['id']}.json", "w") as t:
                        print(body, file=t)
                ...
                try:
                    ...
                    last_simbol = body["question"][-1]
                    base_question = body["question"][:-1]
                    trim_question = re.sub(r'[、。?？！!]', "", base_question)
                    body["question"] = trim_question + last_simbol

                    ret = favot.execute(body)
                    ret, ret_debug = ret
                    ret_ = [i[0].replace(trim_question, base_question) for i in ret.most_common(10)[:10]]
                    if ret is not None:
